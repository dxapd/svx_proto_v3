...
Layout::Tree::Tree(int viewportWidth,
                   int viewportHeight)
    : boxes() {
    Layout::Box viewportBox =
        Layout::Box{Layout::Kinds::Free{}, std::monostate{}, Content::Fit::Fill,
                    Pixels{viewportWidth}, Pixels{viewportHeight}};

    boxes.emplace_back(viewportBox);
    viewportBoxHandle = 0;
}

Layout::BoxHandle& Layout::Tree::GetViewportBox() { return viewportBoxHandle; }
...
size_t Layout::Tree::Count() const { return boxes.size(); }