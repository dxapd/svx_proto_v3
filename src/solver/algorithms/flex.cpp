...
void SolveLayout(
    ...
    std::vector<Solver::FinalBox>& finalBoxes) {
        ...
        for (const Layout::Child<Layout::Kinds::FlexParams>& child : layout.children) {
            const Layout::Box& childBox = layoutTree.GetBox(child.box);
            if (std::holds_alternative<Layout::Units::Auto>(childBox.size[mainAxis])) {
                autoWeights += child.params.weight;
            }
            else {
                leftoverExtent -= Solver::ResolveSize(childBox.size[mainAxis], parentMainExtent);
            }
        }
        ...
        for (const Layout::Child<Layout::Kinds::FlexParams>& child : layout.children) {
            ...
            if (std::holds_alternative<Layout::Units::Auto>(childBox.size[mainAxis])) {
                ...
                childExtent = (int) ((child.params.weight / autoWeights) * leftoverExtent);
            }
            else {
                childExtent = Solver::ResolveSize(childBox.size[mainAxis], parentMainExtent);
            }
            ...
            if (std::holds_alternative<Layout::Units::Auto>(childBox.size[crossAxis])) {
                finalChild.extent[crossAxis] = parent.extent[crossAxis];
            }
            else {
                finalChild.extent[crossAxis] = Solver::ResolveSize(childBox.size[crossAxis], parentCrossExtent);
            }
