...
Solver::FinalTree::FinalTree() : finalBoxes() {
    rootBoxHandle = -1;
}

void Solver::FinalTree::Reset() {
    finalBoxes.clear();
}
...
Solver::FinalBoxHandle Solver::FinalTree::AddBox(Solver::FinalBox finalBox, Solver::FinalBoxHandle parentHandle) {
    finalBoxes.emplace_back(finalBox);
...
Solver::FinalBox& Solver::FinalTree::GetBox(Solver::FinalBoxHandle finalBoxHandle) {
    return finalBoxes[finalBoxHandle];
}

Solver::FinalBoxHandle Solver::FinalTree::GetRootBox() {
    return rootBoxHandle;
}